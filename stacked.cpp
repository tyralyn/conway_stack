...
void runEverytime(){
  testX++; 
  if (testX%100 != 0 || testX == zDim-2) return;
  cout<<"-----"<<testX/100<<"-----"<<endl;
  //bool B = false;
  for (int i= 1; i < xDim-1; i++) {
    for (int j = 1; j < yDim-1; j++) {
      //for (int k = 1; k < zDim -1; k++) {
        int n = numLiveNeighbors(i,j,testX/100);
        bool b = isAlive(i,j,testX/100);
        if (b) {
          //cout<<values[k](i,j)<<" "<<i<<" "<<j<<" "<<k<<" is alive and has neigbors "<<n<<endl;
          if (n >= el && n <= eu) {
            liveCell(i,j,testX/100);
            //cout<<"living  "<<i<<" "<<j<<" "<<k<<" with "<<n<<endl;
          }
          else {
            killCell(i,j,testX/100);
...
{   
  ...
  // Other Transformations
    ...
    for (int i = 0; i < xDim; i ++) {
      for (int j = 0; j < yDim; j++) {
        for (int k = 0; k < zDim; k++) {
          if (isAlive(i,j,k) ) drawCube(i-xDim/2,j-yDim/2,k-zRes/2, 0.25, (float)(i+10.0)/(float)xDim, (float)(j+10.0)/(float)yDim, (float)(k+10.0)/(float)zDim);
        }
      }
    }
  }
